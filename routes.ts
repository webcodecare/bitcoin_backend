...
import { insertUserSchema, insertSignalSchema, insertTickerSchema, insertUserAlertSchema, insertDashboardLayoutSchema } from "./schema";
...
import { authenticateToken, requireSubscription, requireFeature, requireAdmin, requirePayment } from "./middleware/subscriptionAuth";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Advanced cycle forecasting endpoint - requires pro tier
  app.post('/api/forecast/advanced/:ticker', requireAuth, requireSubscription('pro'), async (req, res) => {
    try {
      const { ticker } = req.params;
  ...
  app.post("/api/create-subscription", requireAuth, async (req: any, res) => {
    try {
      ...
      // For demo purposes, directly update user subscription
      ...
      await storage.createAdminLog({
        ...
        notes: `Upgraded to ${planTier} plan (${billingInterval})`,
      });
