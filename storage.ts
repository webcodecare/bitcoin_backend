...
let db: any = null;
const databaseUrl = process.env.DATABASE_URL || "postgresql://neondb_owner:npg_C1UoJ2dMrKpl@ep-empty-snow-af97a6i0.c-2.us-west-2.aws.neon.tech/neondb?sslmode=require";
if (databaseUrl) {
...
export function initializeDatabase() {
  ...
  console.log('Initializing database connection - URL source:', process.env.DATABASE_URL ? 'DATABASE_URL' : 'REPLIT_DB_URL');
  console.log('Database URL present:', !!dbUrl);
  
  if (dbUrl) {
    try {
      // Check if it's the Replit KV URL (not PostgreSQL)
      if (dbUrl.includes('kv.replit.com')) {
        console.log('❌ Detected Replit KV URL (not PostgreSQL). Database features will be disabled.');
        console.log('Please set up a proper PostgreSQL database connection.');
        return false;
      }
      
      // Validate and normalize database URL format for Neon
      console.log('Database URL format:', dbUrl.startsWith('postgresql://') ? 'PostgreSQL standard' : 'Custom format');
    ...
    console.log('❌ No database URL found (checked DATABASE_URL and REPLIT_DB_URL)');
    console.log('Database features will be disabled');
    return false;
  }
}
...
export function createTemporaryDbConnection() {
  ...
  if (dbUrl && !dbUrl.includes('kv.replit.com')) {
    try {
      ...
      console.error('Failed to create temporary database connection:', error.message);
      return null;
    }
  }
  return null;
